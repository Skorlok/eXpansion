...
class ColorChooser extends Control implements \ManiaLivePlugins\eXpansion\Gui\Structures\ScriptedContainer
{
    ...
    public function __construct($inputboxName, $sizeX = 35, $digits = 3, $hasPrefix = true, $color = null)
    {
        ...
        $frame->setContent('
        <frame>
            <quad id="preview_' . $buttonId . '" posn="0 0 5" sizen="4 4" halign="left" valign="center" bgcolor="' . (($color != null) ? (ltrim($color, '$')) : ('000')) . '" scriptevents="1" class="colorchooser"/>
            <frame posn="4 0 1">' . \ManiaLivePlugins\eXpansion\Gui\Elements\Inputbox::getXML($inputboxName, $sizeX - 4, true, null, $color, "output_" . $buttonId, "color_input") . '</frame>
            <frame posn="6 4 10">
    ...
    public static function getXML($inputboxName, $sizeX = 35, $digits = 3, $hasPrefix = true, $color = null)
    {
        /** @var Config $config */
        $config = Config::getInstance();
        ...
        $xml = '<frame>
            <quad id="preview_' . $buttonId . '" posn="0 0 5" sizen="4 4" halign="left" valign="center" bgcolor="' . (($color != null) ? (ltrim($color, '$')) : ('000')) . '" scriptevents="1" class="colorchooser"/>
            <frame posn="4 0 1">' . \ManiaLivePlugins\eXpansion\Gui\Elements\Inputbox::getXML($inputboxName, $sizeX - 4, true, null, $color, "output_" . $buttonId, "color_input", false) . '</frame>
            <frame posn="6 4 10">
                <quad id="bg_' . $buttonId . '" posn="-2 2 0" sizen="64 42" bgcolor="222" hidden="1" class="colorSelection"/>
                <quad id="chooser_' . $buttonId . '" posn="0 0 1.0E-5" sizen="32 32" halign="left" valign="top" image="' . $config->colorPreview . '" scriptevents="1" hidden="1" class="colorSelection"/>
                <quad id="hue_' . $buttonId . '" posn="36 0 2.0E-5" sizen="4 32" halign="left" valign="top" image="' . $config->colorHue . '" scriptevents="1" hidden="1" class="colorSelection"/>
