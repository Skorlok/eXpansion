...
class MxUpdate extends Window
{
    ...
    public function update()
    {
        ...
        $this->pager->addItem(new MxInfo(0, "Recieving maps info, please wait...", $this->sizeX - 6));
        ...
        if (count(array_keys($this->mapsToProcess)) >= 1) {
            ...
            $this->clearItems();
            $this->pager->addItem(new MxInfo(0, "Error, not enough maps.", $this->sizeX - 6));
            $this->redraw($this->getRecipient());
        }
    }
    ...
    public function xUpdate($job, $jobData)
    {
        ...
        if ($code !== 200) {
            $this->pager->addItem(new MxInfo(0, "search returned a http error " . $code, $this->sizeX - 6));
            $this->redraw();
            return;
        }
        ...
        if (array_key_exists($this->processedCounter, $this->mapsToProcess)) {
            ...
            $this->pager->addItem(new MxInfo(0, "Processing chunk " . ($this->processedCounter + 1) . "/" . count(array_keys($this->mapsToProcess)), $this->sizeX - 6));
            $this->redraw();
        } else {
            $this->redrawData();
        }
    ...
    public function redrawData()
    {
        ...
        $isadmin = AdminGroups::hasPermission($login, Permission::MAP_ADD_MX);
        ...
        if ($x == 0) {
            $this->pager->addItem(new MxInfo(0, "All maps up-to-date!", $this->sizeX - 6));
        }
        $this->redraw($this->getRecipient());
    }
