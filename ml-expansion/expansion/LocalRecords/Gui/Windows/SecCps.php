...
use ManiaLivePlugins\eXpansion\LocalRecords\Gui\Controls\SecCpItem;
use ManiaLivePlugins\eXpansion\LocalRecords\LocalRecords;

/**
 * Description of SecCps
 *
 * @author Skorlok
 */
class SecCps extends \ManiaLivePlugins\eXpansion\Gui\Windows\Window
{
    private $frame;
    private $label_rank;
    private $label_nick;
    ...
    public function populateList($recs)
    {
        ...
        foreach ($recs as $rec) {
            $this->items[$x] = new SecCpItem($x, $login, $rec, $this->widths, 0);
            $x++;
        }
        $this->updatePage(0);
    }
    ...
    public function updatePage($offset)
    {
        ...
        foreach ($this->recs as $rec) {
            $this->items[$x] = new SecCpItem($x, $this->getRecipient(), $rec, $this->widths, $offset);
