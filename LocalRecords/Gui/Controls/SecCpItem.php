...
/**
 * Description of SecCpItem
 *
 * @author Skorlok
 */
class SecCpItem extends \ManiaLivePlugins\eXpansion\Gui\Control
{
        ...
        for ($x = $offset; $x <= $offset + 6; $x++) {
            ...
            if (array_key_exists($x, $cpArray)) {
                if ($x == 0) {
                    ...
                    $label = new \ManiaLib\Gui\Elements\Label(15, 6);
                    $label->setText(\ManiaLive\Utilities\Time::fromTM($cpArray[$x] - $cpArray[$x - 1]));
                    $label->setAlign("left", "center");
                    $frameCP->addComponent($label);
                }
            }
